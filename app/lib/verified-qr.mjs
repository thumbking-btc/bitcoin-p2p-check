...
import { THUMBKING_QR_LOGO_RGBA_BASE64 } from "./thumbking-qr-logo.mjs";
...
function getLogo() {
  ...
  let binary;
  try {
    binary = globalThis.atob(THUMBKING_QR_LOGO_RGBA_BASE64);
  } catch {
    fail("QR_BRAND", "QR 로고 데이터를 해석하지 못했습니다.");
  }

  if (binary.length < 4 || binary.length % 4 !== 0) {
    fail("QR_BRAND", "QR 로고 픽셀 형식을 확인하지 못했습니다.");
  }
  const pixelCount = binary.length / 4;
  const size = Math.sqrt(pixelCount);
  if (!Number.isInteger(size) || size < 8 || size > 512) {
    fail("QR_BRAND", "QR 로고 이미지 크기를 확인하지 못했습니다.");
  }
  ...
  cachedLogo = Object.freeze({ pixels, width: size, height: size });
  return cachedLogo;
}
...
function overlayLogo(baseRaster, ratio) {
  ...
  for (let y = 0; y < targetSize; y += 1) {
    const sourceY = Math.min(logo.height - 1, Math.floor((y * logo.height) / targetSize));
    const destinationY = startY + y;
    if (destinationY < 0 || destinationY >= baseRaster.height) continue;
    for (let x = 0; x < targetSize; x += 1) {
      const sourceX = Math.min(logo.width - 1, Math.floor((x * logo.width) / targetSize));
      const destinationX = startX + x;
      if (destinationX < 0 || destinationX >= baseRaster.width) continue;
      const sourceOffset = (sourceY * logo.width + sourceX) * 4;
      const alpha = logo.pixels[sourceOffset + 3] / 255;
      if (alpha <= 0) continue;
      const destinationOffset = (destinationY * baseRaster.width + destinationX) * 4;
      const inverseAlpha = 1 - alpha;
      data[destinationOffset] = Math.round(logo.pixels[sourceOffset] * alpha + data[destinationOffset] * inverseAlpha);
      data[destinationOffset + 1] = Math.round(logo.pixels[sourceOffset + 1] * alpha + data[destinationOffset + 1] * inverseAlpha);
      data[destinationOffset + 2] = Math.round(logo.pixels[sourceOffset + 2] * alpha + data[destinationOffset + 2] * inverseAlpha);

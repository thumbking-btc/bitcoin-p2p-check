...
function ReceiveInfoPanel({ expectedSats }: { expectedSats: number | null }) {
  ...
  function build() {
    ...
    try {
      ...
      if (lightningMode === "address") {
        ...
        setFeedback("라이트닝 주소와 주소 QR을 만들었습니다. 인보이스는 생성하지 않습니다.");
        return;
      }
  ...
  return (
    <section style={{ marginBottom: 12, border: "1px solid var(--control-line)", background: "#fff" }}>
      <button
        type="button"
        onClick={() => setOpen((value) => !value)}
        aria-expanded={open}
        style={{ width: "100%", minHeight: 48, padding: "10px 12px", border: 0, background: "#fff", color: "var(--ink)", textAlign: "left", fontWeight: 820, cursor: "pointer" }}
      >
        {open ? "BTC 받을 정보 닫기" : "BTC 받을 정보 추가"}
      </button>

      {open ? (
        <div className={styles.panel} style={{ borderTop: "1px solid var(--control-line)" }}>
          <p style={{ margin: 0, fontSize: 11, color: "var(--muted)" }}>구매자만 입력합니다. 현재 거래의 받을 금액은 <b>{expectedSats ? formatSats(expectedSats) : "계산 전"}</b>입니다.</p>

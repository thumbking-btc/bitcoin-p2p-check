...
export function P2PLightningRequest({ fundingSource, paymentKrw, quoteCurrent, quoteKey, referenceTime, sats }: Props) {
  ...
  useEffect(() => {
    ...
    const timeout = window.setTimeout(() => {
      clearSensitive(false);
      setFailure("안전하게 전달할 남은 시간이 1분 미만입니다. 지갑에서 새 인보이스를 만들어 주세요.");
  ...
  async function buildRequest() {
    ...
    try {
      ...
      if (!mountedRef.current || generationRef.current !== generation || snapshot.quoteKey !== quoteKey || !hasPrivateRequestExportWindow(expiresAt)) {
        qr.data.fill(0);
        return;
      }
  ...
  async function copyInvoice() {
    ...
    if (!externalShareConfirmed) {
      setFailure("클립보드에 전체 인보이스가 남을 수 있다는 내용을 먼저 확인해 주세요.");
      return;
    }
  ...
  async function downloadQr() {
    ...
    const canvas = canvasRef.current;
    if (!externalShareConfirmed) {
      setFailure("다운로드 폴더나 클라우드에 전체 인보이스가 남을 수 있다는 내용을 먼저 확인해 주세요.");
      return;
    }
  ...
  function resumeSensitiveView() {
    ...
    if (inputRef.current) inputRef.current.value = invoiceInput;
    sensitiveSurfaceRef.current?.removeAttribute("hidden");
    setSensitiveViewSuspended(false);
    setExternalShareConfirmed(false);
    setFailure("");
    setFeedback("인보이스·금액과 외부 반출 위험을 다시 확인한 뒤 계속하세요.");
  }
        ...
        <>
          ...
          {artifact ? (
            <div ref={resultRef} className="receive-artifact">
              ...
              <label className="receive-external-confirmation">
                ...
                <span>복사·공유·저장하는 모든 결과에는 전체 인보이스·금액이 포함되고, 1:1 이미지와 텍스트에는 자금 출처도 포함되며, 외부 앱·클립보드·다운로드 폴더·클라우드에 남아 회수되지 않을 수 있음을 확인했습니다.</span>
              </label>
              <div className="receive-export-actions">
                ...
                <button type="button" onClick={() => void copyInvoice()} disabled={!externalShareConfirmed}>BOLT11 인보이스 복사</button>
          ...
          {sensitiveViewSuspended ? (
            <div className="receive-privacy-cover" role="status">
              ...
              <p>지갑 앱이나 공유 창을 여는 동안 인보이스와 QR을 가렸습니다. 값은 요청 만료 전까지 이 탭의 메모리에만 유지됩니다.</p>
              <div className="receive-actions">

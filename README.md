...
- 실시간 가격 연결이 정상일 때 프리미엄과 수수료 참고값은 약 5분마다 갱신합니다. WebSocket 연결이 끊기면 `/api/market?price=1`을 사용한 업비트 REST 가격 조회가 60초 주기로 자동 복귀합니다.
...
- WebSocket 연결이 끊기면 화면이 보이고 온라인인 동안 15초, 30초, 60초 순서로 재연결을 시도하고 이후에는 60초 간격을 유지합니다. 백그라운드 또는 오프라인 상태에서는 재연결을 시도하지 않습니다.

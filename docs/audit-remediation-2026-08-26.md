...
| H-02 / F-02 preview 격리 | 별도 preview Worker를 사용하고 production KV·서명 secret을 선언하지 않으며 모든 record mutation을 fail closed로 처리합니다. 실제 Dashboard binding 상속 여부는 외부 확인 항목입니다. | 코드 완료·외부 검증 필요; 원래 주장은 조건부 |
...
| 항목 | 조치 | 상태 |
| --- | --- | --- |
| M-01 / F-07 Lightning 남용 | fail-closed 전용 limiter, 정확한 media type, 엄격한 주소·callback 관계, 공개 HTTPS 제한, bounded body, 0/1/2 redirect와 초과·429·timeout 경계시험을 추가했습니다. 보고서의 “요청당 최대 두 외부 호출” 표현은 부정확했고, redirect를 포함하면 더 많은 hop이 가능했습니다. | 자동 검증 완료 |
...
| M-12 / F-18 복구 | signed absolute expiry를 조회·복원 후에도 강제하고 RPO/RTO, absolute expiration, revoke/deletion 전파, quarantine restore 절차를 문서화했습니다. 암호화 backup 목적지와 service identity는 운영자가 선택·구성해야 합니다. | 외부 결정·운영 필요 |
...
| M-22 배포 경쟁 | production concurrency와 최신 `main` SHA guard를 추가했습니다. 자동 rollback은 KV·binding·key 호환성을 판단하지 못한 채 상태를 악화시킬 수 있어 강제하지 않고, 승인형 rollback 절차로 유지합니다. | 자동 검증 완료; canary/rollback 실행은 외부 운영 필요 |
...
1. Cloudflare Git 직접 배포·branch preview 해제와 production/preview binding 실측

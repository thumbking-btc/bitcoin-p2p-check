...
test("receive info exposes only a verified, current, amount-snapshotted result", async () => {
  ...
  assert.match(component, /normalizeLightningAddress\(source\.trim\(\)\)\.address/u);
  assert.match(component, /createOnchainRequest\(target\.address, BigInt\(amountSats\)\)/u);
...
test("holds the visible quote while exact-amount payment information is active", async () => {
  ...
  assert.match(calculator, /if \(!info\) return;[\s\S]*paymentLockRef\.current = true/u);
  assert.match(calculator, /if \(state\.status !== "empty"\)[\s\S]*paymentLockRef\.current = true[\s\S]*paymentLockRef\.current = false/u);
  assert.match(calculator, /if \(!preparedTradeShareRef\.current && !paymentLockRef\.current\)[\s\S]*const pendingSnapshot = pendingMarketSnapshotRef\.current/u);
  assert.match(calculator, /paymentReferenceLocked[\s\S]*"금액 고정 중"/u);
  assert.match(calculator, /const receiveConditionKey = JSON\.stringify\(\{ tradeRole, sats: quote\?\.sats \?\? null \}\)/u);
  assert.match(calculator, /rail: verifiedReceiveInfo\.rail,[\s\S]*payload: verifiedReceiveInfo\.payload,[\s\S]*address: verifiedReceiveInfo\.address,[\s\S]*expiresAt: verifiedReceiveInfo\.expiresAt/u);
});

...
test("accepts five canonical mainnet types and creates one minimal BIP 321 URI", () => {
  ...
  for (const [address, scriptType, scriptPubKeyHex] of ADDRESSES.slice(2)) {
    const uppercase = address.toUpperCase();
    assert.deepEqual(decodeKnownMainnetAddress(uppercase), {
      canonicalAddress: address,
      scriptType,
      scriptPubKeyHex,
    });
    assert.equal(build(uppercase).address, address);
  }
});
...
test("rejects and identifies likely secret material before address handling", () => {
  ...
  assert.equal(assertP2PReceiveAddressInputSafe(ADDRESSES[4][0].toUpperCase()), ADDRESSES[4][0].toUpperCase());
});
...
test("payment request flows are buyer-authorized, local-only, short-lived, and outside public promotion sharing", async () => {
  ...
  assert.match(wrapper, /props\.transferSupport === "both" \? null : props\.transferSupport/);
  ...
  for (const sensitiveComponent of [component, lightning]) {
    ...
    assert.match(sensitiveComponent, /sensitiveSurfaceRef\.current\?\.setAttribute\("hidden", ""\)/);
    assert.match(sensitiveComponent, /document\.visibilityState === "hidden" && snapshot/);
    assert.match(sensitiveComponent, /window\.addEventListener\("pageshow", handlePageShow\);\s*handleVisibility\(\)/);
    assert.match(sensitiveComponent, /const handlePageHide = \(\) => clearSensitive\(false\)/);
    assert.match(sensitiveComponent, /disabled=\{!externalShareConfirmed\}/);
    assert.ok((sensitiveComponent.match(/if \(!externalShareConfirmed\)/g) ?? []).length >= 4);
    assert.match(sensitiveComponent, /hasPrivateRequestExportWindow\(candidate\.source\.expiresAt\)/);
    assert.match(sensitiveComponent, /!hasPrivateRequestExportWindow\(snapshot\.expiresAt\)/);
    assert.match(sensitiveComponent, /artifact\.source\.expiresAt - PRIVATE_REQUEST_EXPORT_MARGIN_MS - Date\.now\(\)/);
    assert.match(sensitiveComponent, /artifactRef\.current\?\.source\.expiresAt \?\? snapshot\?\.expiresAt/);
  }
  assert.match(lightning, /minimumRemainingSeconds: 120/);
  assert.doesNotMatch(worker, /pendingSnapshotWithPrice|pendingReferenceSnapshot/);
  for (const sensitiveComponent of [component, lightning]) {

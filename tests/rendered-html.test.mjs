...
test("keeps market data official and separates public promotion from private requests", async () => {
  ...
  assert.match(css, /\.amount-unit-control\s*\{[^}]*min-height:\s*44px/s);
  assert.match(css, /\.amount-unit-control\s*\{[^}]*width:\s*58px;[^}]*min-width:\s*58px/s);
  assert.match(css, /\.amount-unit-select\s*\{[^}]*font:\s*780 11px\/1\.3 var\(--sans\)/s);
  assert.match(css, /\.amount-unit-chevron\s*\{[^}]*font:\s*900 15px\/1 var\(--sans\)/s);
  assert.match(css, /\.premium-stepper\s*\{[^}]*width:\s*58px;[^}]*min-width:\s*58px[^}]*grid-template-rows:\s*repeat\(2/s);
  ...
  assert.match(component, /suppressNextResultAnnouncementRef\.current = silent;\s*if \(silent\) setResultAnnouncement\(""\);\s*marketRef\.current = nextData;/);
  assert.match(component, /suppressNextResultAnnouncementRef\.current = true;\s*setResultAnnouncement\(""\);\s*marketRef\.current = nextData;/);
